...
import CertificateList from '../../Components/ShowCertificate'
...
const Certificates = () => {
  ...
  const benefits = [
    {
      icon: <Shield className={`w-8 h-8 ${isDark ? "text-yellow-400 " : "" }`} />,
      title: 'Industry Recognition',
      description: 'Our certificates are recognized by top companies and hiring managers worldwide',
    },
    {
      icon: <TrendingUp className={`w-8 h-8 ${isDark ? "text-yellow-400 " : "" }`} />,
      title: 'Career Advancement',
      description: 'Boost your resume and increase your chances of landing your dream job',
    },
    {
      icon: <Users className={`w-8 h-8 ${isDark ? "text-yellow-400 " : "" }`}/>,
      title: 'Skill Validation',
      description: 'Demonstrate your expertise with verified assessments from industry experts',
    },
    {
      icon: <Globe className={`w-8 h-8 ${isDark ? "text-yellow-400 " : "" }`}/>,
      title: 'Global Standards',
      description: 'Certificates based on international standards and best practices',
    },
  ];
  ...
  const earnedCertificates = [
    {
      ...
      viewUrl: 'https://www.africau.edu/images/default/sample.pdf',
      imageUrl: 'https://via.placeholder.com/600x400.png?text=Technical+Skills+Certificate'
    },
    {
      id: 2,
      ...
      issuer: 'SkillCorp',
      skills: ['Written Communication', 'Presentation', 'Team Work']
    }
  ];
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-black text-white' : 'bg-white text-black'}`}>
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Hero Section */}
        <div className="text-center mb-16">
          <div className={`inline-flex items-center justify-center w-20 h-20 bg-yellow-400/10 rounded-full border-2 ${isDark ? "border-yellow-400/20" : "border-gray-400 bg-yellow-100"} mb-6`}>
            <Award className="w-10 h-10 text-yellow-500" />
          </div>
          <h1 className="text-5xl font-bold mb-4">
            Earn <span className="text-yellow-400" style={{ WebkitTextStroke: '1px #000' }}>Industry-Recognized</span> Certificates
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center my-8">
            <button 
              ...
              className="bg-yellow-400 text-black hover:bg-yellow-300 px-6 py-3 text-lg rounded-md flex items-center font-medium">
              <Trophy className="w-5 h-5 mr-2" />
              View My Certificates
              <ArrowRight className="w-5 h-5 ml-2" />
            </button>
            <button
              className={`border border-yellow-400 ${isDark ? "text-yellow-400" : "bg-yellow-200" } hover:bg-yellow-400 hover:text-black px-6 py-3 text-lg rounded-md flex items-center font-medium`}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              ...
              <div key={i}>
                <div className={`${isDark ? "text-yellow-400 " : " "} text-3xl font-bold `}>{count}</div>
            ...
            {benefits.map((b, i) => (
              <div
                ...
                className={`rounded-lg border border-yellow-500/20 p-4 text-center group hover:scale-105 transition-all duration-300 ${
                  isDark ? 'bg-black/20' : 'bg-yellow-400 '
                }`}
              >
                <div className="flex justify-center mb-4">
                ...
                <h3 className={`text-lg ${isDark ? "text-yellow-400 " : "text-black "} font-semibold mb-3`}>{b.title}</h3>
        ...
        <div className="mb-16">
          <h2 className="text-3xl font-bold text-center mb-12">Certificate Sample</h2>
          <div className={`flex justify-center `}>
            <div className={`bg-white text-black p-8 rounded-lg shadow-lg max-w-2xl w-full ${isDark ? " " : "border-2 border-gray-400"} `}>
              <div className="flex justify-between items-start mb-6">
                <div className="text-blue-600 font-bold text-2xl">Galo Score</div>
                <div className="font-bold text-xl">Certified</div>
              </div>
        ...
        <div className="text-center">
          ...
          <div className="grid md:grid-cols-4 gap-8 mb-12">
            {['Choose Program', 'Complete Assessment', 'Meet Requirements', 'Receive Certificate'].map((step, i) => (
              <div key={i}>
                <div className="group">
                  <div className={`w-16 h-16 ${isDark ? "bg-[rgba(245,158,11,0.1)] " : "bg-yellow-300 " } rounded-full mx-auto border-2 border-yellow-400/20 flex items-center justify-center mb-4 transition-all duration-300 group-hover:bg-yellow-400 group-hover:scale-110 `}>
                    <span className={`text-2xl font-bold ${isDark ? "text-yellow-400 " : ""} group-hover:text-black transition-colors duration-300`}>
                      {i + 1}
                    </span>
                  </div>
                </div>
